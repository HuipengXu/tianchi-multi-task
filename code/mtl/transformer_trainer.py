...
from mtl.trainer_utils import PREFIX_CHECKPOINT_DIR, EvalPrediction, PredictionOutput, TrainOutput
from mtl.utils import task_id_to_name, task_num_classes, EMA
from mtl.model import BertMultiTaskModel
from mtl.args import DataTrainingArguments, ModelArguments
...
class Trainer:
    ...
    def _setup_wandb(self):
        ...
        args = vars(self.args)
        args.update(vars(self.model_args))
        args.update(vars(self.data_args))
        wandb.init(dir=os.path.dirname(self.args.logging_dir), project=os.getenv("WANDB_PROJECT", "huggingface"), config=args)
        # keep track of model topology and gradients
        if os.getenv("WANDB_WATCH") != "false":
            wandb.watch(
                self.model, log=os.getenv("WANDB_WATCH", "gradients"), log_freq=max(100, self.args.logging_steps)
            )
        # self.args.output_dir = wandb.run.dir
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > self.args.save_steps and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        if self.args.evaluate_during_training:
                            ...
                            avg_score = sum(metrics[metric] for metric in
        ...
        return TrainOutput(self.global_step, tr_loss / self.global_step), model, best_score
    ...
    def save_model(self, output_dir: Optional[str] = None):
        """
        Saving best_model-practices: if you use default names for the model,
    ...
    def predict(self, test_dataset: Dataset) -> PredictionOutput:
        ...
        if self.args.do_train:
            self.model = BertMultiTaskModel.from_pretrained(self.best_model_path, task_num_classes=task_num_classes,
